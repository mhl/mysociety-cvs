...
class TextLayer(TileCache.Layer.MetaLayer):
    ...
    def renderTile(self, tile, force=None):
        ...
        datapoints = [(transform(*bng2gym(x, y)), t)
                      for (x, y, t) in get_data(self.results_id, xmin, ymin, xmax, ymax)]
        ...
        image.save(buffer, self.extension)
        buffer.seek(0)
        tile.data = buffer.read()
        return tile.data 
        
        
        raise Exception(str((xscale, xoffset, yscale, yoffset)) + ', ' + str(datapoints))

        raise Exception(str(get_data(self.results_id, xmin, ymin, xmax, ymax)))
        
        raise Exception('(%(xmin)f %(ymin)f %(xmax)f %(ymax)f)' % locals())
...
def get_data(results_id, xmin, ymin, xmax, ymax):
    """
    """
    # open the file
