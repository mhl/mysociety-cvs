...
//
// $Id: makeplan.cpp,v 1.1 2009-03-10 20:58:04 francis Exp $
//
...
class PlanningATCO {
    ...
    std::map<LocationID, std::set<JourneyID> > journeys_visiting_location;
    ...
    */
    ...
    void load_binary_timetable(const std::string& l_in_prefix) {
        ...
        for (int i = 1; i <= this->number_of_journeys; ++i) {
            ...
            printf("%s hops:%d\n", j->toString().c_str(), number_of_hops);
    ...
    */
    void adjacent_location_times(Adjacents &adjacents, Location target_location, Minutes target_arrival_time) {
