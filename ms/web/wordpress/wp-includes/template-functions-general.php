...
function get_archives($type='', $limit='', $format='html', $before = '', $after = '', $show_post_count = false, $cat = 1) {
	...
	if ( 'monthly' == $type ) {
		$arcresults = $wpdb->get_results("SELECT DISTINCT YEAR(post_date) AS `year`, MONTH(post_date) AS `month`, count(ID) as posts FROM $wpdb->posts LEFT JOIN $wpdb->post2cat ON $wpdb->posts.ID = post_id WHERE post_date < '$now' AND post_date != '0000-00-00 00:00:00' AND post_status = 'publish' AND category_id = $cat GROUP BY YEAR(post_date), MONTH(post_date) ORDER BY post_date DESC" . $limit);
		if ( $arcresults ) {
			$afterafter = $after;
		...
		$arcresults = $wpdb->get_results("SELECT * FROM $wpdb->posts LEFT JOIN $wpdb->post2cat ON $wpdb->posts.ID = post_id WHERE post_date < '$now' AND post_status = 'publish' AND category_id = $cat ORDER BY post_date DESC" . $limit);
		if ( $arcresults ) {
			foreach ( $arcresults as $arcresult ) {
